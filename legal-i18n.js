...
const ugcSections = {
  es: ['8. Comunidad, comentarios y noticias', '<p>Las publicaciones, comentarios y Me gusta son contenido social visible según la privacidad de cada cuenta. Puedes denunciar publicaciones, comentarios o usuarios desde la app. Revs puede ocultar o eliminar contenido y limitar cuentas para prevenir abuso. Las noticias muestran un resumen elaborado por Revs a partir de fuentes oficiales, citan la fuente y permiten activar o desactivar el aviso diario.</p>'],
  en: ['8. Community, comments and news', '<p>Posts, comments and likes are social content shown according to each account\'s privacy. You can report posts, comments or users in the app. Revs may hide or remove content and restrict accounts to prevent abuse. News includes a Revs summary based on official sources, credits the source and lets you enable or disable the daily alert.</p>'],
  fr: ['8. Communauté, commentaires et actualités', '<p>Les publications, commentaires et mentions J’aime sont affichés selon la confidentialité de chaque compte. Vous pouvez signaler une publication, un commentaire ou un utilisateur. Revs peut masquer ou supprimer du contenu et limiter des comptes pour prévenir les abus. Les actualités comprennent un résumé Revs fondé sur des sources officielles, citent la source et permettent d’activer ou de désactiver l’alerte quotidienne.</p>'],
  de: ['8. Community, Kommentare und Nachrichten', '<p>Beiträge, Kommentare und „Gefällt mir“-Angaben werden gemäß der Privatsphäre des jeweiligen Kontos angezeigt. Beiträge, Kommentare und Nutzer können in der App gemeldet werden. Revs kann Inhalte ausblenden oder entfernen und Konten zum Schutz vor Missbrauch einschränken. Nachrichten enthalten eine Revs-Zusammenfassung aus offiziellen Quellen, nennen die Quelle und bieten eine ein- oder ausschaltbare tägliche Benachrichtigung.</p>'],
  it: ['8. Community, commenti e notizie', '<p>Post, commenti e Mi piace sono contenuti social visibili in base alla privacy di ciascun account. Puoi segnalare post, commenti o utenti dall’app. Revs può nascondere o rimuovere contenuti e limitare account per prevenire abusi. Le notizie includono un riepilogo Revs basato su fonti ufficiali, citano la fonte e consentono di attivare o disattivare l’avviso giornaliero.</p>'],
  pt: ['8. Comunidade, comentários e notícias', '<p>Publicações, comentários e gostos são conteúdos sociais apresentados conforme a privacidade de cada conta. Podes denunciar publicações, comentários ou utilizadores na aplicação. A Revs pode ocultar ou remover conteúdos e limitar contas para evitar abusos. As notícias incluem um resumo da Revs baseado em fontes oficiais, citam a fonte e permitem ativar ou desativar o aviso diário.</p>'],
  nl: ['8. Community, reacties en nieuws', '<p>Berichten, reacties en vind-ik-leuks worden getoond volgens de privacy van elk account. Je kunt berichten, reacties of gebruikers in de app melden. Revs kan inhoud verbergen of verwijderen en accounts beperken om misbruik te voorkomen. Nieuws bevat een Revs-samenvatting op basis van officiële bronnen, vermeldt de bron en laat je de dagelijkse melding in- of uitschakelen.</p>'],
  fi: ['8. Yhteisö, kommentit ja uutiset', '<p>Julkaisut, kommentit ja tykkäykset näkyvät kunkin tilin tietosuoja-asetusten mukaisesti. Voit ilmoittaa julkaisuista, kommenteista tai käyttäjistä sovelluksessa. Revs voi piilottaa tai poistaa sisältöä ja rajoittaa tilejä väärinkäytösten estämiseksi. Uutisissa on virallisiin lähteisiin perustuva Revs-yhteenveto, lähdeviite ja valinnainen päivittäinen ilmoitus.</p>'],
  sv: ['8. Community, kommentarer och nyheter', '<p>Inlägg, kommentarer och gilla-markeringar visas enligt varje kontos integritetsinställningar. Du kan rapportera inlägg, kommentarer eller användare i appen. Revs kan dölja eller ta bort innehåll och begränsa konton för att förebygga missbruk. Nyheter innehåller en Revs-sammanfattning baserad på officiella källor, anger källan och låter dig slå på eller av den dagliga aviseringen.</p>'],
  zh: ['8. 社区、评论和新闻', '<p>帖子、评论和点赞会按照各账户的隐私设置显示。你可以在应用内举报帖子、评论或用户。为防止滥用，Revs 可以隐藏或删除内容并限制账户。新闻包含基于官方来源生成的 Revs 摘要、明确的来源标注，以及可开启或关闭的每日通知。</p>'],
  ja: ['8. コミュニティ、コメント、ニュース', '<p>投稿、コメント、いいねは各アカウントの公開範囲に従って表示されます。アプリ内で投稿、コメント、ユーザーを報告できます。Revs は不正利用防止のため、コンテンツの非表示・削除やアカウント制限を行う場合があります。ニュースには公式情報源に基づく Revs の要約と出典が表示され、毎日の通知はオン・オフできます。</p>'],
  pl: ['8. Społeczność, komentarze i wiadomości', '<p>Posty, komentarze i polubienia są widoczne zgodnie z prywatnością danego konta. W aplikacji można zgłaszać posty, komentarze i użytkowników. Revs może ukrywać lub usuwać treści oraz ograniczać konta, aby zapobiegać nadużyciom. Wiadomości zawierają podsumowanie Revs oparte na oficjalnych źródłach, wskazanie źródła i opcjonalne codzienne powiadomienie.</p>'],
  ru: ['8. Сообщество, комментарии и новости', '<p>Публикации, комментарии и отметки «Нравится» показываются с учётом настроек приватности аккаунта. В приложении можно пожаловаться на публикацию, комментарий или пользователя. Для предотвращения злоупотреблений Revs может скрывать или удалять контент и ограничивать аккаунты. Новости содержат краткое изложение Revs на основе официальных источников, ссылку на источник и отключаемое ежедневное уведомление.</p>']
};
...
function render() {
  ...
  if (page === 'privacy') {
    ...
    const sections = [...item.sections, ugcSections[language] || ugcSections.en];
    document.title = `${item.title} · Revs`;
    content.innerHTML = `<h1>${item.title}</h1><p class="updated">${item.updated}</p><p>${item.intro}</p>${sections.map(section => `<h2>${section[0]}</h2>${section[1]}`).join('')}<div class="card"><strong>${item.contact}</strong><p><a href="mailto:danieluriaf@gmail.com">danieluriaf@gmail.com</a></p></div>`;
  } else {
